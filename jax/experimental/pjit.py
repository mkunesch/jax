...
def pjit(fun: Callable,
  ...
  ``pjit`` takes care to automatically partition the inputs in that case.
  However, for best performance it is important to ensure that the arguments
  are correctly sharded (e.g. they are output of another ``pjit`` with
  ``out_axis_resources`` matching the desired ``in_axis_resources``), in which
  case no transfers will be performed outside of the compiled computation.
